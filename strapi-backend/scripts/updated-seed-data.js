...
const existingData = {
  ...
  blogPosts: [
    {
      title: "5 Legal Mistakes Early-Stage Startups Make",
      slug: "5-legal-mistakes-early-stage-startups-make",
      description: "Learn about the most common legal pitfalls that can slow down your startup and how to avoid them.",
      content: "# 5 Legal Mistakes Early-Stage Startups Make\n\nStarting a company is exciting, but legal missteps can be costly. Here are the top 5 mistakes we see founders make:\n\n## 1. Skipping Founder Agreements\n\nMany startups begin without clear founder agreements, leading to disputes later...",
      publishedDate: "2024-01-15T10:00:00.000Z",
      author: "Junes Legal",
      readTime: "5 min read",
      tags: ["legal", "startups", "founders"]
    },
    {
      title: "Understanding Belgian Company Structure: BV vs NV",
      slug: "belgian-company-structure-bv-vs-nv",
      description: "A comprehensive guide to choosing the right legal structure for your Belgian startup.",
      content: "# Understanding Belgian Company Structure: BV vs NV\n\nChoosing the right legal structure is crucial for your Belgian startup...",
      publishedDate: "2024-02-01T10:00:00.000Z",
      author: "Junes Legal",
      readTime: "8 min read",
      tags: ["belgium", "incorporation", "legal structure"]
    }
  ],
  careerPosts: [
    {
      title: "Junior Legal Associate",
      slug: "junior-legal-associate",
      location: "Brussels, Belgium",
      type: "full-time",
      description: "Join our growing team as a Junior Legal Associate and help shape the future of startup legal services.",
      fullDescription: "Join our growing team as a Junior Legal Associate and help shape the future of startup legal services. You'll work closely with our experienced legal team to support early-stage startups with their legal needs.",
      datePosted: "2024-12-01T10:00:00.000Z",
      requirements: [
        "Law degree from a recognized university",
        "0-2 years experience in corporate law",
        "Strong interest in startups and technology",
...
async function seedData() {
  try {
    console.log('🌱 Starting data seeding via PUBLIC API...');

    // Update Hero Section (single type - use PUT)
    console.log('📝 Updating hero section...');
    try {
      await axios.put(`${STRAPI_URL}/api/hero-section`, {
        data: { ...existingData.heroSection, publishedAt: new Date().toISOString() }
      }, { headers });
      console.log('✅ Hero section updated');
    } catch (error) {
      console.log('⚠️  Hero section error:', error.response?.status, error.response?.data?.error?.message);
    }

    // Update About Section (single type - use PUT)
    console.log('📝 Updating about section...');
    try {
      await axios.put(`${STRAPI_URL}/api/about-section`, {
        data: { ...existingData.aboutSection, publishedAt: new Date().toISOString() }
      }, { headers });
      console.log('✅ About section updated');
    } catch (error) {
      console.log('⚠️  About section error:', error.response?.status, error.response?.data?.error?.message);
    }

    // Create/Update Service Plans (collection type - use POST)
    ...
    for (const plan of existingData.servicePlans) {
      try {
        ...
        console.log(`⚠️  Service plan "${plan.name}" error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }

    // Create Why Work Features (collection type - use POST)
    ...
    for (const feature of existingData.whyWorkFeatures) {
      try {
        ...
        console.log(`⚠️  Why work feature "${feature.title}" error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }

    // Update Pitch Coaching (single type - use PUT)
    console.log('📝 Updating pitch coaching section...');
    try {
      await axios.put(`${STRAPI_URL}/api/pitch-coaching`, {
        data: { ...existingData.pitchCoaching, publishedAt: new Date().toISOString() }
      }, { headers });
      console.log('✅ Pitch coaching section updated');
    } catch (error) {
      console.log('⚠️  Pitch coaching error:', error.response?.status, error.response?.data?.error?.message);
    }

    // Create FAQs (collection type - use POST)
    ...
    for (const faq of existingData.faqs) {
      try {
        ...
        console.log(`⚠️  FAQ error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }

    // Update Founder Section (single type - use PUT)
    console.log('📝 Updating founder section...');
    try {
      await axios.put(`${STRAPI_URL}/api/founder-section`, {
        data: { ...existingData.founderSection, publishedAt: new Date().toISOString() }
      }, { headers });
      console.log('✅ Founder section updated');
    } catch (error) {
      console.log('⚠️  Founder section error:', error.response?.status, error.response?.data?.error?.message);
    }

    // Create Testimonials (collection type - use POST)
    ...
    for (const testimonial of existingData.testimonials) {
      try {
        ...
        console.log(`⚠️  Testimonial from "${testimonial.name}" error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }

    // Create Blog Posts (collection type - use POST)
    console.log('📝 Seeding blog posts...');
    for (const post of existingData.blogPosts) {
      try {
        await axios.post(`${STRAPI_URL}/api/blog-posts`, {
          data: { ...post, publishedAt: new Date().toISOString() }
        }, { headers });
        console.log(`✅ Blog post "${post.title}" seeded`);
      } catch (error) {
        console.log(`⚠️  Blog post "${post.title}" error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }

    // Create Career Posts (collection type - use POST)
    console.log('📝 Seeding career posts...');
    for (const career of existingData.careerPosts) {
      try {
        await axios.post(`${STRAPI_URL}/api/career-posts`, {
          data: { ...career, publishedAt: new Date().toISOString() }
        }, { headers });
        console.log(`✅ Career post "${career.title}" seeded`);
      } catch (error) {
        console.log(`⚠️  Career post "${career.title}" error:`, error.response?.status, error.response?.data?.error?.message);
      }
    }
    ...
    try {
      const heroResponse = await axios.get(`${STRAPI_URL}/api/hero-section`);
    ...
    try {
      const plansResponse = await axios.get(`${STRAPI_URL}/api/service-plans`);
    ...
    try {
      const blogsResponse = await axios.get(`${STRAPI_URL}/api/blog-posts`);
      console.log('✅ Blog posts API:', blogsResponse.data?.data?.length || 0, 'items found');
    } catch (error) {
      console.log('❌ Blog posts API error:', error.response?.status);
    }

    try {
      const careersResponse = await axios.get(`${STRAPI_URL}/api/career-posts`);
      console.log('✅ Career posts API:', careersResponse.data?.data?.length || 0, 'items found');
    } catch (error) {
      console.log('❌ Career posts API error:', error.response?.status);
    }
